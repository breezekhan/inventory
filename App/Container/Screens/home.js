...
import { getCompId, getUserId,getPackageId,getSocietyId, setSocietyId } from '../../Utils/MasterFunctions';
...
//   const [data, setData] = useState([]);
  ...
  const [notes, setComNotes] = useState([]);
  ...
  const [modalVisibleShopping2, setmodalVisibleShopping2] = useState(false)
...
//       requestData.append('company_id', compId);
//       requestData.append('startfrom', 1);
//       requestData.append('totalrecords', 10);
//       const response = await axios.post(BaseUrl + API.Pro_Home, requestData, {
//         headers: {
    ...
    <View style={styles.Container}>
      <View style={styles.Header}>
        <View style={styles.row_header}>
          <Image source={ logo === null ? Images.LOGO : { uri: logo }}
            style={{ height: hp('8%'), width: wp('16%'), resizeMode: 'contain' }} />
          <View style={{ width: wp('60%') }} >
            <Text numberOfLines={2} style={styles.text_header}>Hello,  <Text style={styles.txt_name}>{user.name}</Text></Text>
          {/* <TouchableOpacity onPress={() => { orEmptyString(compy.website) ? null : Linking.openURL(compy.website) }}> */}
              <Text numberOfLines={2} style={styles.text_header}>{compy.name}</Text>
            {/* </TouchableOpacity> */}
          </View>
        </View>
        <TouchableOpacity style={styles.bell} onPress={() => navigation.navigate("notification")}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Fontisto name="bell" color={Colors.white} size={hp('3%')} />
            {notes.length > 0 && (
              <View
                style={{
                  backgroundColor: 'red',
                ...
                <Text style={{ color: Colors.white, fontSize: 10, fontFamily: Fonts.Lato_Reguler }}>{notes.length >= 10 ? "10+" : notes.length}</Text>
              </View>
            )}
          </View>
        </TouchableOpacity>
...
<TouchableOpacity
                ...
                <Image source={{ uri: item.thumbail_image }} style={[styles.box_img]} />
                <View style={[styles.box_trans]}>
                  <View style={[styles.row, { flex: 1 }]}>
                    <Text style={[styles.txt_price, { textDecorationLine: item?.discount_price?.length !== '0' && item?.discount_price > '0' ? 'line-through' : null }]}>₹{item.price}</Text>
                    {item?.discount_price?.length > '0' && item?.discount_price > '0' &&
                      <Text style={[styles.txt_price, { marginLeft: wp('1%') }]}>₹{item.price - item.discount_price}</Text>
                    }
                  </View>
